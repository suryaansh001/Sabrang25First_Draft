...
export default function CheckoutPage() {
    ...
    setSelectedEvents(prev => {
      if (prev.includes(eventId)) {
        ...
        if (hasTimeConflict) {
          ...
          alert(`Cannot select this event. You already have "${conflictingEventData?.title}" selected at ${event.time} on ${event.date}. Please deselect the conflicting event first.`);
          return prev;
        }
  ...
  const getConflictMessage = (eventId: number) => {
    ...
    if (conflictingEvent) {
      const conflictingEventData = events.find(e => e.id === conflictingEvent);
      return `Time conflict with ${conflictingEventData?.title}`;
    }
    return '';
  };
                        ...
                        <div className="bg-blue-500/10 border border-blue-400/30 rounded-lg p-4 mb-4">
                          ...
                          <div className="text-xs text-blue-200">
                            {selectedEvents.map(id => {
                              ...
                              return event ? `${event.title} (${event.date} ${event.time})` : '';
                            }).join(', ')}
                          </div>
                        </div>
                      )}
